...
export class ContainerComponent implements OnInit {
  ...
  addNewsToWatchlist(news) {
    ...
    this.newsService.addNewsToWatchList(news).subscribe(news =>{
      this.matSnackBar.open(message, '', { duration: 10000 });
    },
    error =>{
      this.matSnackBar.open(error['error'], '', { duration: 10000 });
    });
  }
  ...
  deleteFromWatchlist(news) {
    ...
    this.newsService.deleteFromMyWatchlist(news).subscribe(result =>{
      this.matSnackBar.open(message, '', { duration: 8000 });
      ...
      if(this.newsList.length == 0) {
        ...
        this.matSnackBar.open(message, '', { duration: 12000 });
      }
    });
  }
